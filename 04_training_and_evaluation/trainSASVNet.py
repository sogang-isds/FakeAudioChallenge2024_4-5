...
def write_list_to_file(data_list, output_file):
...
def main_worker(args):
    ...
    if args.eval == True:
        ...
        sc, lab ,ids= trainer.evaluateFromList(**vars(args))
        ...
        if args.scoring == True:
            ...
            output_file= 'exp/result/ids.txt'
            write_list_to_file(ids,output_file)
    ...
    for it in range(it,args.max_epoch+1):
        ...
        if it % args.test_interval == 0:
            sc, lab, ids = trainer.evaluateFromList(epoch=it, **vars(args))

            spf_eer = get_all_EERs(sc, lab)
            SPF_EERs += [spf_eer]

            print('\n',time.strftime("%Y-%m-%d %H:%M:%S"), "Epoch {:d}, ACC {:2.2f}, TLOSS {:f}, LR {:2.8f}, SPF_EER {:2.4f}, BestSPF_EER {:2.4f}".format(it, traineer, loss, lr, spf_eer, min(SPF_EERs)))
            scorefile.write("Epoch {:d}, ACC {:2.2f}, TLOSS {:f}, LR {:2.8f}, SPF_EER {:2.4f}, BestSPF_EER {:2.4f}\n".format(it, traineer, loss, lr, spf_eer, min(SPF_EERs)))
            scorefile.flush()
